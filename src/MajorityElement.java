/**
Given an array of size n, find the majority element. The majority element is the element that appears more than O(n/2) times.
...
2. Average runtime: O(n), Worst case runtime: Infinity ！ Randomization: 
   ...
   As the probability to pick the majority element is greater than 1/2, the expected number 
   of attempts is < 2.
...
3. Runtime: O(n log n) ！ Divide and conquer: Divide the array into two halves, 
   ...
   in the second half. The global majority element must either be A or B. 
   If A == B, then it automatically becomes the global majority element. 
   ...
   The runtime complexity, T(n) = T(n/2) + 2n = O(n logn).
...
5. Runtime: O(n) ！ Bit manipulation: We would need 32 iterations, each calculating 
   the number of 1's for the ith bit of all n numbers. 
   Since a majority must exist, therefore, either count of 1's > count of 0's or vice versa 
   (but can never be equal). The majority number¨s ith bit must be the one bit that has 
   the greater count.
...
public class MajorityElement {
	...
	public static int majorityElement2(int[] num) {
		...
		for (int elem: hash.keySet()) {
			if (hash.get(elem) > num.length/2)
				return elem;
		}
		return -1;
